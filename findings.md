...
### 2026-09-01 execution-case screenshot — semantic diagram mismatch

- The screenshot is an execution-mechanics lesson: one case describes a buy limit order behind an 80-lot queue and a 20-lot intended order; the other describes a stop trigger at 98 followed by thin bid depth and a 97.6 average fill.
- Neither mechanism is encoded by the current picture. Both figures reuse the same price-path polyline with candle marks and lower bars, so the learner cannot see queue priority, remaining quantity, trigger versus fill price, depth consumed, or slippage.
- The three-stage reading order is useful and should remain, but it cannot substitute for a domain model. The correct repair is `concept → diagram family → case-specific state`, not `case → differently colored line`.
- Relevant visual families include: OHLC candle anatomy, parent/child timeframe nesting, bid/ask queue and depth ladder, data-source pipeline, risk ruler and payoff geometry, session timeline, position-size staircase, swing/zone structure map, liquidity pool and sweep/acceptance, range coordinates, market/volume profile, footprint/Delta cells, and DOM event replay.
- A small renderer registry is preferable to 57 bespoke SVGs: each lesson selects a semantically appropriate family, while its valid/failed/conflict outcome and three-stage prose remain data-driven.
- The course already defines one interactive concept diagram for 20 of 21 lessons (`candle-anatomy`, `timeframe-zoom`, `spread-auction`, `risk-ruler`, `footprint-grid`, `volume-profile`, and others). Those mappings provide the correct semantic vocabulary for the static case plates; only `hybrid-workflow` needs a dedicated workflow-gate mapping.
- The current case profile table already centralizes all 21 lesson ids, so it can become the diagram registry without adding per-case conditionals throughout the page renderer.
- Fixed 720-unit SVG labels previously rendered too small inside 448px two-up cards. New concept diagrams need a larger label scale, fewer annotations, and direct labels on the mechanism rather than tiny captions around a generic plot.
- The profile registry now maps every lesson id to one of 19 semantic renderers. Execution, provenance, risk, session, exposure, workflow, structure, liquidity, zones, nesting, range, orderbook, footprint, Delta, profile, auction-extreme, and DOM diagrams no longer depend on a shared price-series pattern.
- Each mechanism plate keeps three stable columns and stage headers, but the content inside those columns is domain-specific. This preserves the successful reading-order improvement without pretending that every concept is a time-series problem.
- Narrow-card container rules increase SVG label sizes while stacking the prose stages. Wide cards retain compact labels and a three-column comparison, so the semantic diagram remains legible in both layouts.
- The first runtime contract check on `order-basics` confirms two `diagram-execution` figures, one explicit queue graphic, three explicit depth rows for stop slippage, three prose stages per case, and no desktop overflow. Visual inspection still needs the page to finish its asynchronous content layout before scrolling to the figures.
- The final execution screenshot now directly shows `80 手在前 → 90 手卖单打入 → 仅成交 10/20 手` and `Stop 98 → 三档 Bid 深度 → 均价 97.6 / 滑点 0.4`. This is the intended semantic improvement over two unrelated price lines.
- The narrow queue segment exposed one small label collision (`我 20` inside a 40-unit box); the box now shows only the quantity `20`, while the surrounding text already identifies it as the learner's order.
- Runtime sampling confirms the expected semantic classes for risk, footprint, volume-profile, and DOM routes. Their figures preserve all three prose stages and none of the sampled desktop routes overflows.
- As with earlier deep-course pages, the first screenshot immediately after navigation can land above the casebook while asynchronous lesson content settles. Visual QA must wait once more and then recalculate the figure's live position before capture.
- The Footprint screenshot passes visually: the first column is a real Bid × Ask matrix, the second highlights the `Ask ≥ 3× Bid` relationship, and the third explicitly compares price confirmation (`离开关键区`) with conflict (`收在中部`). No generic price line is used.
- A batched screenshot attempt for risk, profile, and DOM again captured pre-case sections because each route's interactive lesson diagram expands after the first scroll. Subsequent captures should target one route at a time and perform the same delayed second-position calculation used successfully for execution and Footprint.
- The DOM screenshot passes visually: the mechanism is now three directly comparable ladder snapshots (`出现 → 事件 → 结果`). Replenishment shows 40 → 300 traded → 40 restored; the counterexample shows 1,000 → 0 → unknown intent, making the evidentiary boundary visible without a price line.
- A single delayed risk capture still landed at the terminology primer, so the risk route needs its live scroll geometry inspected before another capture rather than another fixed-delay retry.
- Live geometry showed the risk figure had already reached the intended 120px viewport offset after the screenshot race. Capturing the settled state confirms the mechanism works: planned 2R/3R geometry, realized win rate and average win, and cost-aware expectancy are connected in one reading path; the failed example visibly ends at `−0.49R`.
- Volume Profile visual QA passes in a wide textbook case: a historical distribution, a developing distribution with highlighted POC/HVN, and the resulting acceptance classification are shown as horizontal price-volume histograms rather than a price line.
- At 390px the execution plate remains within a 388px figure, uses 20-unit SVG labels, stacks the three prose stages at full width, and has no horizontal overflow. The diagram is intentionally compact context; the large prose rows remain the primary mobile explanation.
- Full desktop sweep covers all 21 lessons and 57 figures: all 57 use `.case-concept-svg`, all retain three visual stages plus three prose stages, no legacy `.case-path-line` remains, no SVG text box falls outside the 720×360 drawing area, and no route overflows.
- The sweep confirms 19 visible diagram families across the curriculum: OHLC, timeframe, execution, provenance, risk, session, Martingale, workflow, structure, liquidity, zones, nesting, range, orderbook, footprint, Delta, profile, auction-extreme, and DOM.
- Full 390×844 sweep also passes: 57 figures, zero missing stages, every prose strip stacked into one column, all small SVG labels promoted to 20 user units, and zero overflowing routes.
- A semantic audit after the visual sweep found three concepts where outcome color alone was insufficient: `auction-engine` has two valid cases but the second is absorption; `liquidity-map` may validly classify either Sweep or Acceptance; and `volume-profile` must distinguish genuine value migration, transient early POC movement, and conflicting TPO/volume evidence.
...
- The referenced desktop design list contains a second requirement beyond concept-specific plates: one case should expose the causal bridge from context and structural hypothesis through liquidity/flow evidence to execution and risk. The current 19 renderers satisfy local mechanism relevance but not that cross-system bridge.
- The correct implementation boundary is semantic HTML for the relationship graph and CSS for presentation. A single ordered list can preserve source order and accessibility while the desktop stylesheet controls five- or six-column lanes; duplicating desktop/mobile markup would make explanations diverge.
- The current renderer already has the right reuse boundary: all micro and textbook cases pass through `renderCaseFigure`. Adding one relationship list there will cover all 57 figures without duplicating lesson HTML.
- Existing case presentation is ordered as mechanism SVG → three-stage prose → caption. The missing association layer should precede the SVG so learners first see the causal map, then inspect the concept-specific evidence, then read the temporal explanation.
- Existing container queries are presentation-only and can be retained. They change grid columns and label sizes without changing information or producing a second DOM tree, which matches the requested “HTML semantic, CSS adaptation” architecture.
- The 42 microcases plus 15 textbook cases already cover the necessary causal variants: confirmation, failure, and conflict are present for structure, liquidity, PD arrays, confluence, profile, and execution. The association layer can therefore be lesson-driven while its terminal node remains case-outcome-driven.
- A five-stage chain is the clearest common denominator across all lesson families: `context → hypothesis/object → observable evidence → decision gate → execution/risk`. Labels must change per lesson (for example, queue/depth for execution and POC/value for profile) so the shared HTML does not become generic copy.
- Fresh 1440×900 browser inspection of `order-basics` confirms the semantic order is exposed correctly: each case presents the five-node causal bridge, the execution-specific queue/depth plate, the three temporal stages, and the detailed facts/reasoning in that order.
- With the course directory collapsed, the content area is 898px and the CSS produces five equal 179.6px nodes with 15px body copy; the document remains exactly 1440px wide with zero horizontal overflow. The microcase grid is one 898px column rather than two squeezed columns.
- The first 1440px screenshot shows the execution plate itself is now directly related and legible, but also exposes a desktop-only opportunity: focus mode leaves roughly 370px of unused space to the right while the teaching figure remains 898px wide. Since mobile is out of scope, the focused lesson column should use more of the desktop canvas before increasing diagram density further.
- Removing the focused lesson body's old 1120px cap through a sidebar-collapsed CSS rule expands the case map from 898px to 1160px at 1440×900. Each causal node grows from about 180px to 232px, explanatory copy increases to 16px, and the page still has zero horizontal overflow.
- Visual inspection now shows one coherent desktop dossier: five directly labeled causal nodes, the execution queue/depth plate, the three-stage narrative, and the facts/reasoning ledger share the same full-width reading column. No alternative HTML tree was added for this layout.
- Representative browser checks confirm the chains are genuinely lesson-specific: Hybrid uses `HTF Bias → Liquidity + POI → Sweep/MSS/Retest → Footprint/Delta`; Footprint uses `结构假设 → Bid×Ask → Imbalance → 价格确认`; Risk uses `失效条件 → 止损距离 → 仓位与成本 → 期望值闸门`; Profile uses historical value, departure, center migration, and acceptance.
- The full desktop sweep covers 21 routes and 57 figures: every figure has one five-node map, all 21 lessons have a unique causal signature, no map has a missing node, no route overflows, and the console has no warnings or errors.
